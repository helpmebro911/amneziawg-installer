...
При подключении смартфонов через сотовую связь могут быть проблемы: соединение устанавливается, но трафик не идёт. На iPhone ситуация хуже — может не подключиться вообще.

**Причины:**

1. **Высокий MTU тоннеля.** WireGuard по умолчанию использует MTU 1420. Сотовые сети (4G/LTE) часто имеют эффективный MTU ниже — пакеты фрагментируются или отбрасываются. iOS особенно строго обрабатывает Path MTU Discovery.

2. **Большие junk-пакеты.** Параметр Jmax может достигать ~1000 байт. В сочетании с MTU 1420 на сотовой связи пакеты могут не проходить.

**Решение:**

Добавьте `MTU = 1280` в секцию `[Interface]` серверного и клиентского конфигов:
...
1280 — минимальный MTU для IPv6 (RFC 8200), проходит через любую сеть. На скорость влияет минимально (чуть больше служебных пакетов из-за меньшего размера payload).

> **Примечание:** В vpn:// URI для Amnezia Client MTU = 1280 уже установлен по умолчанию. Проблема проявляется при использовании `.conf`-файлов с нативным AmneziaWG-клиентом на смартфоне.

После изменения MTU перезапустите сервис:
...
| [AmneziaWG](https://github.com/amnezia-vpn/amneziawg-android) | Android | ✅ | ✅ | Нативный WG-клиент с AWG. Использует `.conf` |
...
| amneziawg-windows-client | Windows | ✅ | ❌ | Только AWG 1.x! Ошибка «Неверный ключ: S3» |
...
* **MTU по умолчанию не задан в .conf.** Для мобильных клиентов рекомендуется вручную добавить `MTU = 1280` (см. раздел [MTU и мобильные клиенты](#mtu-mobile-adv)).
